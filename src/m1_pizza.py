...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(100, 200),
                rg.Point(200, 200),
                rg.Point(150, 150),
                rg.Point(150, 250),
                rg.Point(114.6, 285.4),
                rg.Point(264.6, 285.4)]
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  8 pink dots.'
    window = rg.RoseWindow(350, 350, title)
    circle = rg.Circle(rg.Point(280, 310), 27)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    dots = generate_points_on_circle(circle, number_of_points)
    for k in range(len(dots)):
        scircle = rg.Circle(dots[k], 10)
        scircle.fill_color = color
        scircle.attach_to(window)
        point = scircle.center
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 4:  100 slices, thin (thickness=3) yellow lines.'
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(dots)):
        line = rg.Line(circle.center, dots[k])
        line.color = color
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON tests 3:  4 segments with medium black lines.'
    window = rg.RoseWindow(400, 400, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(dots) - 1):
        line = rg.Line(dots[k], dots[k + 1])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    fline = rg.Line(dots[0], dots[len(dots)-1])
    fline.color = color
    fline.thickness = thickness
    fline.attach_to(window)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(0, len(dots)):
        if k + a < number_of_lines:
            line = rg.Line(dots[k], dots[k + a])
            line.color = color
            line.thickness = thickness
            line.attach_to(window)
        if k + a >= number_of_lines:
            line = rg.Line(dots[k], dots[(k + a) % number_of_lines])
            line.color = color
            line.thickness = thickness
            line.attach_to(window)
